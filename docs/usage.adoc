...
For an ID provider that wants to ship its own stylesheet or icon set with the login page, pair lib-static with lib-router inside the controller: one route serves static, the others handle login / logout / etc.
...
----
+
. *Add the controller.* The `GET` export dispatches via lib-router. A real ID provider would also export `login` / `logout` / `handle401`; those are left out here — this example only shows the static-asset wiring.
+
